...
func TestPasteTemplateShowsFullLoadConfirmationForTruncatedContent(t *testing.T) {
	...
	body := output.String()
	for _, want := range []string{`id="loadFullButton"`, "Load full paste", `window.confirm("Load everything?")`, `id="remainingPasteContent"`, `id="remainingLinesNotice"`, "... (125 more lines)", "remainingLinesNotice.remove()", `id="qrButton"`, `data-src="?format=qr"`, `id="lineHighlight"`, "focusLineFromHash", "data-line", "paste_policy: manage_policy_temporary", "paste_expires:", "2026-09-12T12:00:00"} {
